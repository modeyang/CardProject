...
#include "BHGX_MF_HCT.h"
#include "dcrf32/headers/dcrf32.h"
#include "debug.h"

#pragma  comment(lib, "dcrf32/libs/dcrf32.lib")
...
//add by yanggx
//增加的读卡密码模型
#define DEFAULT_CONTROL	0		//默认配置 KeyA读 KeyB写  双密码时
#define KEYA_CONTROL	1		//KeyA读写 单密码时， KeyA有所有权限，KeyB无效
#define KEYB_CONTROL	2		//KeyB读写 这种暂不实现，保留
int g_Control = DEFAULT_CONTROL;

//这个函数时为了在验证写密码时，返回的的写控制字，用于dc_load_key第二个参数
...
static void SetKey(struct CreateKeyInfoS *key)
{
	...
	*/
	...
	if(result)
		DBG(0, "[SetKey 关键信息]: Authentication=%d\n", result);
	...
	for(secs = 0; secs < 16; secs++)
	{
		...
		if(result)
			DBG(0, "[SetKey 密码]: %d Authentication=%d\n", secs,result);
...
int __stdcall iRead(const unsigned char *key,unsigned char *buf, int iSizeInBits, int iOffsetInBits)
{
	...
	while(iSizeInBits != 0) {
		...
		if(result)
			DBG(0, "[Read]: Authentication=%d\n", result);

		// 读取一整块数据
		if((BitNr==0) && (iReadDataInBits == 128))
		{
			result = dc_read(mGHandle, BlkNr, now);
		}
		// 读取部分数据
		else 
		{
			memset(data, 0, 16);
			result = dc_read(mGHandle, BlkNr, data);
					 ...
					 int nCtrlWord,int iSizeInBits, int iOffsetInBits)
{
	...
	while(iSizeInBits != 0)
	{
		...
		if(result)
			DBG(0, "[Write]: Authentication = %d\n", result);

		// 整块写入
		if((BitNr == 0) && (iWriteDataInBits == 128))
		{
			result = dc_write(mGHandle, BlkNr, now);
		}
		// 部分写入
		else 
		{
			...
			result = dc_write(mGHandle, BlkNr, data);
		}
							...
							)
{
	...
	result = dc_read(mGHandle, BlkNr, OldKey);
