...
int __stdcall apt_ScanCard(char *card_info)
{
	...
	if (cardType == CARDTYPE_CPU16 || cardType == CARDTYPE_CPU32) {
		...
		for (; seat <= CARDSEAT_PSAM2; seat++) {
			int seat_status = Instance->ICCSet(seat, &psamType, resp);
			LOG_INFO("SAM卡寻找,槽位号:%d, status:%d, type:%d, sam info:%s", seat, seat_status, psamType, resp);
	...
	if (status != 0 || (cardType != eM1Card && seat_scan_status == 0)) {
		LOG_ERROR("寻卡错误, card type: %d status: %d, seat_scan_status: %d", cardType, status, seat_scan_status);
		return CardScanErr;
	}
