...
static int _HospReadInfo(int section, char *xml);
...
{
	int status = 0;
	status = _HospReadInfo(SEG_CLINICINFO, xml);
	return status;
}
...
{
	int status = 0;
	status = _HospReadInfo(SEG_HOSPINFO, xml);
	return status;
}
...
{
	int status = 0;
	status = _HospReadInfo(SEG_FEEINFO, xml);
	return status;
}
